...
import { ICompanyBankMySuffix, defaultValue } from 'app/shared/model/company-bank-my-suffix.model';
...
const initialState = {
  loading: false,
  errorMessage: null,
  entities: [] as ReadonlyArray<ICompanyBankMySuffix>,
  entity: defaultValue,
  updating: false,
  updateSuccess: false
};

export type CompanyBankMySuffixState = Readonly<typeof initialState>;

// Reducer

export default (state: CompanyBankMySuffixState = initialState, action): CompanyBankMySuffixState => {
  switch (action.type) {
...
export const getEntities: ICrudGetAllAction<ICompanyBankMySuffix> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_COMPANYBANK_LIST,
  payload: axios.get<ICompanyBankMySuffix>(`${apiUrl}?cacheBuster=${new Date().getTime()}`)
});

export const getEntity: ICrudGetAction<ICompanyBankMySuffix> = id => {
  const requestUrl = `${apiUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_COMPANYBANK,
    payload: axios.get<ICompanyBankMySuffix>(requestUrl)
  };
};

export const createEntity: ICrudPutAction<ICompanyBankMySuffix> = entity => async dispatch => {
...
export const updateEntity: ICrudPutAction<ICompanyBankMySuffix> = entity => async dispatch => {
  const result = await dispatch({
...
export const deleteEntity: ICrudDeleteAction<ICompanyBankMySuffix> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
